...
def replace_linear_with_quantized(model: nn.Module,
                                ...
                                num_bits: int = 1,
                                group_size: int = 128,
                                initial_scale_value: float = 2.0, # Renamed from init_scale
...
class QuantizedLinear(nn.Linear):
    def __init__(self, 
        ...
        if num_bits == 1:
            # For 1-bit quantization, weights are binarized to {-1, +1}
            self.thd_neg = -1
            self.thd_pos = 1
        else:
        ...
        if group_size is None:
            raise NotImplementedError("group_size must be specified when groupwise is True")
