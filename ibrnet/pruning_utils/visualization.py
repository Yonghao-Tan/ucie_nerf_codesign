...
def visualize_mask_assignment(z_vals_coarse, z_samples, assignment, ray_idx=0, save_path="mask_assignment.png"):
    ...
    Args:
        z_vals_coarse: [N_rays, N_coarse] coarse采样点深度
        z_samples: [N_rays, N_importance] fine采样点深度  
        assignment: [N_rays, N_importance] assignment关系
        ray_idx: 可视化的ray索引
        save_path: 保存路径
    """
    ...
    for i, z in enumerate(z_fine):
        nearest_coarse_idx = assign[i]
        ax.scatter(z, 0.05, c=[colors[i]], s=60, marker='x', alpha=0.8)
        
        # 画连接线显示assignment
        ax.plot([z_coarse[nearest_coarse_idx], z], [0, 0.05], 'r--', alpha=0.5, linewidth=1)
        
        # 标注assignment
        ax.text(z, 0.07, f'{nearest_coarse_idx}', ha='center', fontsize=6, color='red')
    
    ax.scatter([], [], c='red', s=60, marker='x', label=f'Fine samples ({len(z_fine)})')
    
    ax.set_ylim(-0.02, 0.1)
    ax.set_xlabel('Depth (meters)')
    ax.set_title(f'Fine Sample Assignment to Coarse Points (Ray {ray_idx})')
    ax.legend()
