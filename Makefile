...
test:
	cd ./eval && \
	CUDA_VISIBLE_DEVICES=6 python eval.py --config ../configs/eval_llff.txt --num_source_views 8 --sv_prune --sample_point_sparsity --sv_top_k 4 --sample_point_group_size 4

test2:
	cd ./eval && \
	CUDA_VISIBLE_DEVICES=6 python eval.py --config ../configs/eval_llff_sr.txt --num_source_views 8 --resize_factor 0.5 --sr --sample_point_sparsity --use_moe --sv_prune --sv_top_k 4 --sample_point_group_size 4
