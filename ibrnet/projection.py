...
class Projector():
    def __init__(self, device):
        self.device = device
        self.save_idx = 0
        self.pixel_locations_accum = None  # 用于累积保存 pixel_locations
    ...
    def compute_projections(self, xyz, train_cameras):
        ...
        :param xyz: [..., 3]
        :param train_cameras: [n_views, 34], 34 = img_size(2) + intrinsics(16) + extrinsics(16) [extrinsics: c2w]
    ...
    def compute(self,  xyz, query_camera, train_imgs, train_cameras, featmaps):
        ...
        normalized_pixel_locations = self.normalize(pixel_locations, h, w)   # [n_views, n_rays, n_samples, 2] [x, y]
        n_samples = normalized_pixel_locations.shape[2]
        # rgb sampling
        rgbs_sampled = F.grid_sample(train_imgs, normalized_pixel_locations, align_corners=True) # TODO 试一下如果把mask也弄一起 直接置0会有影响吗
        ...
        # print(rgb_feat_sampled.shape, ray_diff.shape, mask.shape)
        # rgb_feat_sampled *= mask # 这里为什么乘了mask之后精度有变化，不过仅低了0.04
        # ray_diff *= mask
        # print(mask.sum() / mask.numel(), inbound.sum() / inbound.numel())
        # print(xyz.shape, train_cameras.shape, query_camera.shape)
        # print(train_cameras[0])
        # print(query_camera)
        # if n_samples == 48: # and mask.sum() / mask.numel() > 0.8:
        #     h_get, w_get = 32, 1008
        #     m = (pixel_locations[..., 0] >= 0) & (pixel_locations[..., 1] >= 0)
        #     print(m.sum() / m.numel())
        #     print(pixel_locations[...,0].min(), pixel_locations[...,0].max(), pixel_locations[...,1].min(), pixel_locations[...,1].max())
        #     np.save('pixel_locations_valid_hr.npy', pixel_locations.detach().cpu().numpy())
        #     pixel_locations_np = np.load('pixel_locations_valid_hr.npy')
        #     print(h, w, pixel_locations.shape, pixel_locations.detach().cpu().numpy().shape, pixel_locations_np.shape)
        #     pixel_locations_np = np.reshape(pixel_locations_np, (10, h_get*w_get, n_samples, 2))
        #     np.save('sample_locations_valid.npy', xyz.detach().cpu().numpy())
        ...
        #     exit()
        if False and n_samples == 16:
            # 累积保存逻辑
            with torch.no_grad():
                cur = pixel_locations.detach().cpu()
                if self.pixel_locations_accum is None:
                    self.pixel_locations_accum = cur
                else:
                    self.pixel_locations_accum = torch.cat([self.pixel_locations_accum, cur], dim=1)
                # 只有 shape[0] != 7560 时才保存
                if cur.shape[1] != 15*train_imgs.shape[-1]: # TODO
                    print('save', cur.shape)
                    torch.save(self.pixel_locations_accum, f"../memory/nerf_synthetic/locations_hr/pixel_locations_{self.save_idx}_n{n_samples}.pt")
