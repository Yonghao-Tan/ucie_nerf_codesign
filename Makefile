...
test2:
	cd ./eval && \
	CUDA_VISIBLE_DEVICES=6 python eval.py --config ../configs/eval_llff_sr.txt --eval_scenes fern --chunk_height 10 --ckpt_path ../pretrained/pretraining/model_best.pth --num_source_views 8 --window_size 5 --sample_point_group_size 8 --sample_point_sparsity --sv_prune --sv_top_k 5 --use_moe --resize_factor 0.5 --sr --new_sr
# --q_bits 8 --sparsity 0.5
test2_nosr:
	cd ./eval && \
...
tests:
	cd ./eval && \
	CUDA_VISIBLE_DEVICES=6 python eval.py --config ../configs/eval_llff_sr.txt --eval_scenes fern --ckpt_path ../pretrained/pretraining/model_best.pth --chunk_height 10 --window_size 5 --num_source_views 8 --resize_factor 0.5 --sr --new_sr
...
trains:
	TORCH_DISTRIBUTED_DEBUG=DETAIL CUDA_VISIBLE_DEVICES=0,1,2,3,4,5 python -m torch.distributed.launch --master_port=33111 --nproc_per_node=6 train.py --config configs/pretrain_sr.txt --ckpt_path pretrained/model_16_32_sr_small.pth --no_load_scheduler --no_load_opt --chunk_height 15 --sample_mode block_single --num_source_views 8 --window_size 5 --resize_factor 0.5 --sr --new_sr --q_bits 8 --sparsity 0.5

# 其实coarse不用prune，本来就很少了