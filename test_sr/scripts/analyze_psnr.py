...
def main():
    ...
    if not os.path.exists(img_dir):
        print(f"图片目录 '{img_dir}' 不存在，请确保data文件夹存在并包含所需图片")
        return
    
    gt_path = os.path.join(img_dir, "no2_gt.png")
    org_path = os.path.join(img_dir, "no2_pred_org.png") 
    sr_path = os.path.join(img_dir, "no2_pred_sr.png")
